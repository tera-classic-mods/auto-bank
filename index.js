...
module.exports = function AutoBank(mod) {
	...
	function sViewWareEx(event) {
		...
		if (!GuildBanking && event.type === 3) return Msg("Banking to guild bank is <font color=\"#FE6F5E\">disabled</font>.");
		let toBankItems = [];
		if (event.items.length > 0 && mod.game.inventory.items.length > 0) {
			...
			for (const toBank of toBankItems)
				mod.toServer("C_PUT_WARE_ITEM", 2, {
					...
					"uid": toBank.dbid,
					"amont": toBank.amount,
					"bankPos": event.offset
				});
		}
		mod.setTimeout(() => {
			if (!Enabled) return;
			if (event.offset + 72 < event.slots)
				mod.toServer("C_VIEW_WARE", 2, {
					"gameId": mod.game.me.gameId,
					"type": event.type,
					"offset": event.offset + 72
				});
			else
	...
	function sRequestContract(event) { if (event.type === 27) curBankContractId = event.id; }
	...
	mod.hook("S_VIEW_WARE_EX", 1, sViewWareEx);
	mod.hook("C_PUT_WARE_ITEM", 2, cPutWareItem);
	...
	const Msg = msg => mod.command.message(msg),
		resetValues = () => {
